...
state_t p2state, p3state, p4state, p5state, p6state, p7state, p8rootstate,
    ...
    gchild4state, p9state, p10state, hp_p1state, hp_p2state;
...
void test()
{
    ...
    pFiveSupport.sup_except_context[GENERALEXCEPT].pc = (memaddr)p5gen;
    pFiveSupport.sup_except_context[PGFAULTEXCEPT].stack_ptr = p5Stack;
    pFiveSupport.sup_except_context[PGFAULTEXCEPT].status =
        ALLOFF | IEPBITON | CAUSEINTMASK | TEBITON;
...
void p5gen()
{
    unsigned int exeCode = pFiveSupport.sup_except_state[GENERALEXCEPT].cause;
    ...
    switch (exeCode) {
        case BUSERROR:
            ...
            pFiveSupport.sup_except_state[GENERALEXCEPT].reg_t9 =
                (memaddr)p5a; /* Continue with p5a() */
            break;
        ...
        case RESVINSTR:
            ...
            pFiveSupport.sup_except_state[GENERALEXCEPT].reg_t9 =
                (memaddr)p5b; /* Continue with p5b() */
            pFiveSupport.sup_except_state[GENERALEXCEPT].status =
                pFiveSupport.sup_except_state[GENERALEXCEPT].status & KUPBITOFF;
            break;
...
void p5mm()
{
    ...
    pFiveSupport.sup_except_state[PGFAULTEXCEPT].status =
        pFiveSupport.sup_except_state[PGFAULTEXCEPT].status |
        KUPBITON; /* user mode on 	*/
    pFiveSupport.sup_except_state[PGFAULTEXCEPT].pc_epc =
        (memaddr)p5b; /* return to p5b()	*/
    pFiveSupport.sup_except_state[PGFAULTEXCEPT].reg_t9 =
        (memaddr)p5b; /* return to p5b()	*/

    LDST(&(pFiveSupport.sup_except_state[PGFAULTEXCEPT]));
}

/* p5's SYS trap handler */
void p5sys()
{
    unsigned int p5status = pFiveSupport.sup_except_state[GENERALEXCEPT].status;
